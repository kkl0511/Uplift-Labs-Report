...
리포트 페이지 우측 상단에 두 가지 공유 버튼이 있습니다.

### ① 🔗 즉시 링크 (권장 · 간단)
- **GitHub 설정 불필요**, 클릭 즉시 생성
- 리포트 데이터를 LZString으로 압축해 URL 자체에 포함
- 형태: `https://<도메인>/#/share/<압축데이터>`
- 평균 URL 크기: 1~6 KB (대부분 메신저에서 정상 동작)
- URL이 8 KB를 넘으면 자동 경고 (카톡 등에서 잘릴 가능성 있음)
- 데이터가 URL에 들어 있어 **서버 없이도 영구 유효**

### ② 🔗 GitHub 링크 (짧은 URL 필요시)
- GitHub Personal Access Token + Pages 배포 필요
- 짧고 깔끔한 URL: `https://<유저>.github.io/<레포>/#/r/<선수명-날짜>`
- 같은 선수·같은 날짜로 다시 분석하면 **기존 링크가 자동 갱신** (URL 재전송 불필요)
- ⚠ Pages 반영에 30~90초 대기

> 💡 **추천**: 일회성 공유는 ①, 장기 보관/북마크용은 ②
